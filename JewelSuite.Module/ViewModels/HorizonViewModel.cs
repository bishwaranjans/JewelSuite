...
namespace JewelSuite.Module.ViewModels
{
    public class HorizonViewModel : BindableBase
    {
        ...
        private double _volumeOfOilAndGasInCubicFeet;
        public double VolumeOfOilAndGasInCubicFeet
        {
            get { return _volumeOfOilAndGasInCubicFeet; }
            set { SetProperty(ref _volumeOfOilAndGasInCubicFeet, value); }
        }

        private double _volumeOfOilAndGasInBarrels;
        public double VolumeOfOilAndGasInBarrels
        {
            get { return _volumeOfOilAndGasInBarrels; }
            set { SetProperty(ref _volumeOfOilAndGasInBarrels, value); }
        }
        ...
        {
            get { return _canShowVolume && TopHorizon != null && TopHorizon.Length != 0; }
        ...
        public DelegateCommand LoadHorizonDataCommand { get; private set; }

        public DelegateCommand ShowVolumeCommand { get; private set; }

        public HorizonViewModel(IApplicationCommands applicationCommands, IVolumeCalculationService volumeCalculationService)
        {
            _applicationCommands = applicationCommands;
            _volumeCalculationService = volumeCalculationService;

            LoadHorizonDataCommand = new DelegateCommand(LoadHorizonData);
            ShowVolumeCommand = new DelegateCommand(ShowVolume).ObservesCanExecute(() => CanShowVolume);

            _applicationCommands.SaveCommand.RegisterCommand(LoadHorizonDataCommand);
        }

        private void LoadHorizonData()
        {
            if (TopHorizon == null || TopHorizon.Length == 0)
            {
                ...
                VolumeOfOilAndGasInCubicMeter = _volumeCalculationService.CalculateOilAndGasVolumeFromTopHorizon(TopHorizon);
            }
        }

        private void ShowVolume()
        {
            Constants.VolumeUnit volumeUnit;
            Enum.TryParse(Title, out volumeUnit);
            ...
            switch (volumeUnit)
            {
                ...
                case Constants.VolumeUnit.CubicFeet:
                    {
                        VolumeOfOilAndGasInCubicFeet = VolumeOfOilAndGasInCubicMeter.ToCubicFeet();
                ...
                case Constants.VolumeUnit.Barrels:
                    {
                        VolumeOfOilAndGasInBarrels = VolumeOfOilAndGasInCubicMeter.ToBarrels();
